...
echo "$(docker run --name datacontainer -d -v wordpress:/var/www/html -v wordpress:/var/lib/mysql/ centos)";
...
echo "$(docker run --link mariadb:mariadb --name httpd --volumes-from datacontainer -d -p 8080:80 jribelles/centos-httpd-php-jribelles)";

echo"$(wget http://wordpress.org/latest.tar.gz)";

echo "$(tar -xzvf latest.tar.gz)";

echo "$(docker cp wordpress datacontainer:/var/www/html)";
